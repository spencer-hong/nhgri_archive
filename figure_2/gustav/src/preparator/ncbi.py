...
def pubmed():
    ...
    files = sorted([x for x in os.listdir(in_dir) if x.endswith('xml.gz')])
    mismatches = []
    ...
    for file in files:
        ...
        if not str(readable_hash) in str(lines):
            print(file)
            mismatches.append(file)

    if any(mismatches):
        print('The following files are corrupted:')
        [print(x) for x in mismatches]

        raise AssertionError('At least one file corrupted')
