...
def seg_swissprot():
    ...
    for taxon in settings.priviledged_organisms:
        df = properties.seg_swissprot(taxon)
        df.columns = [x.replace('SEG_swissprot: ', '') for x in df.columns]

        p = 'access_framework/{}/seg_swissprot/{}.parquet'.format(
            data_version, int(taxon))
        inout.export_plain_table(df, p)
...
def radar_swissprot():
    ...
    for taxon in settings.priviledged_organisms:
        df = properties.radar_swissprot(taxon)
        df.columns = [x.replace('Radar_swissprot_best: ', '') for x in df.columns]

        p = 'access_framework/{}/radar_swissprot/{}.parquet'.format(
            data_version, int(taxon))
        inout.export_plain_table(df, p)


def signalp_swissprot():
    data_version = access_inout.datasets['geisen']

    for taxon in settings.priviledged_organisms:
        df = properties.signalp_swissprot(taxon)
        df.columns = [x.replace('SignalP_swissprot: ', '') for x in df.columns]

        p = 'access_framework/{}/signalp_swissprot/{}.parquet'.format(
            data_version, int(taxon))
        inout.export_plain_table(df, p)
