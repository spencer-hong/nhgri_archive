...
from gustav import ebi, ncbi, utils
...
def to_homolog(df, taxon_ncbi):
    """
    Maps df, with "gene_ncbi" collumn to "taxon_ncbi"
    via homologene. Note that this does not do aggregation
    (e.g.: mean if there are multiple orthologs)

    Input:
        df          dataframe with column "gene_ncbi"
        taxon_ncbi  NCBI taxonomy identifier
    ...
    new= pd.merge(
        hg[['homologene_group', 'gene_ncbi']],
        df.copy()).drop('gene_ncbi', axis='columns')

    df = pd.merge(
        hg[hg['taxon_ncbi']==taxon_ncbi][['gene_ncbi', 'homologene_group']],
        new
    ).drop('homologene_group', axis='columns')
...
def gene_ncbi_2_uniprot_id(df):
    ...
    uniprot = ebi.uniprot(
        'uniprot_to_ncbi_gene', 'vip'
        ).rename(columns={'protein_uniprot': 'uniprot_id'})

    df = pd.merge(
        uniprot[['gene_ncbi', 'uniprot_id']], 
        df).drop('gene_ncbi', axis='columns')
