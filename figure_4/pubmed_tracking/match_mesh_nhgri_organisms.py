...
grouped = mesh.groupby("UI").agg({"value": np.sum})['value'].to_dict()
...
meshs['mesh_terms'] = meshs['mesh_terms'].progress_apply(lambda x: x.split(":")[0].strip().lower())
...
for i, pattern in tqdm(enumerate(isolated_patterns), total = len(term_expanded)):
	...
	together[:, i] = np.array(pubmed['pubmed_id'].isin(meshs.loc[meshs['mesh_terms'].progress_apply(lambda x: False if pattern.search(x) is None else True)]['pubmed_id']))

np.save('../../manuscript/cache/mesh_matched_by_nhgri_organisms', together)