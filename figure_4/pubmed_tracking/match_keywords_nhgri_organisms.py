...
keywords['keywords'] = keywords['keywords'].progress_apply(lambda x: x.lower())
...
for i, pattern in tqdm(enumerate(isolated_patterns), total = len(term_expanded)):
	...
	together[:, i] = np.array(pubmed['pubmed_id'].isin(keywords.loc[keywords['keywords'].progress_apply(lambda x: False if pattern.search(x) is None else True)]['pubmed_id']))


np.save('../../manuscript/cache/keywords_matched_by_nhgri_organisms', together)
