...
pubmed = ncbi.pubmed('main',
                    columns = ['pubmed_id', 'pubdate', 'title'],
...
for i, pattern in tqdm(enumerate(isolated_patterns), total = len(term_expanded)):
	together[:, i] =  np.array(pubmed['title'].progress_apply(lambda x: False if pattern.search(x) is None else True))


np.save('../../manuscript/cache/title_matched_nhgri_organisms', together)